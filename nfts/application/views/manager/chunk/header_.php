...
<head>
    ...
    <title>Artsky - Branding into next level </title>

    <link rel="icon" href="<?=$templates?>images/icon-red.png" type="image/gif" sizes="16x16">

    <meta content="text/html;charset=utf-8" http-equiv="Content-Type">

    <meta content="width=device-width, initial-scale=1.0" name="viewport" />

    <meta content="Artsky - Branding into next level " name="description" />

    <meta content="blockchain, crypto, binance, binacensmartchain, ARTSereum, bitcoin, wallet, nft, art, brand, model" name="keywords" />

    <meta content="artsky" name="author" />
    <base href="<?=base_url()?>">
    ...
    <link href="<?=$templates?>css/assets/font-awesome.min.css" rel="stylesheet" type="text/css" /> 
    ...
    <link href="<?=$templates?>plugin/select2/css/select2.css" rel="stylesheet">  
       <link href="<?=$templates?>plugin/select2/select2-bootstrap.css" rel="stylesheet">   
        <script src="<?=$templates?>plugin/select2/js/select2.js"></script>
    ...
    <script type="text/javascript">
        //moralis server
         ...
         const appId = "8IME6ptizVobA7tMo0W3HXENpuVTiIlSuH9ZIreh";
            ...
            window.vm = new Vue({
                ...
                methods: {
					...
					gettokenbalance: async function () {
						...
						const format = Web3Client.utils.fromWei(balance); //Web3Client.utils.fromWei(balance,'gwei'); //Web3Client.utils.fromWei(balance);
					...
					buying: async function (owners,tokenids,totalbuyings) {
					  ...
					  const alls = ethers.utils.parseUnits((totalbuyings+1+totalfeebuy).toString(), decimalPrecision) ;
					  ...
					  let format = Web3Client.utils.fromWei(balance);
					  let checkapprove_token =  {};
					  let approveResult =  {};
					  let transferfee =  {};
					  let transfer = {};
					  try {
							...
							//
							 checkapprove_token =  await tokencontract.approve(this.account,alls);
							checkapprove_token.wait(1);
							if (typeof checkapprove_token.hash !== "undefined") {
								approveResult = await contractnft.approve(this.account,tokenids);
								...
								//fee
								//transferfee = await tokencontract.transferFrom(this.account,'<?=setting("wallet_fee")?>',bignumber_fees);
								transferfee = await tokencontract.transferFrom(this.account,'<?=setting("wallet_fee")?>',bignumber_fees);
								transferfee.wait(2);
						...
						try {
								...
								transfer = await tokencontract.transferFrom(this.account,owners,bignumber_buyings);
								loading.setMessage('Parse transaction receipt...');
								transfer.wait(2);
								//
					...
					transfer: async function (tokenids) {
						let contractnft = new ethers.Contract(this.getContractAddress(), window.CONTRACT_ABI, window.getWeb3Provider().getSigner());
						const vaddressowner =  await contractnft.ownerOf(tokenids);
						let loading = showLoading('Transfer', 'Verify ...');
						if(this.account.toLowerCase()==vaddressowner.toLowerCase())
						{
							
							var wallet_account = $("#form-transfer #wallet_to").val();
							try {
									//transaction NFT
									 
									loading.setMessage('Parse transaction NFT...');
									const transfernft = await contractnft.transferFrom(vaddressowner,wallet_account,tokenids);
									transfernft.wait(2);
									//
									let url = this.getScanUrlForToken(transfernft.hash);
									loading.close();
									...
									$("#form-transfer #tokenid").val(tokenids);
									$("#form-transfer #wallet_from").val(vaddressowner);
						...
						{
							loading.setMessage('You Are Not Owner');
							$(".modal-backdrop").addClass("hide");
							loading.close();	
						}
					}  
