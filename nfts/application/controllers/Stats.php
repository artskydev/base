...
class Stats extends CI_Controller {
	...
	public function status($code = "")
	{
		...
		if(!empty($code))
		{
			$rec = $this->db->where('activation_code',$code)->get("nft_customer");
			if($rec->num_rows()==1)
			{
				$data = $rec->row_array();
				$this->db->where('activation_code',$code)->update('nft_customer',array('status'=>1));
				$this->session->set_userdata('artsky_nft_login',$data);
				redirect("home");
				return;	
			}
