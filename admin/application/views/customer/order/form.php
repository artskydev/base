            ...
            <div class="card-body">
                                ...
                                <hr/>
                                    ...
                                    <div class="form-group">
                                        <label>Tier</label>
                                        <select name="id_tier" id="id_tier" class="form-control required select2">>
                                        	<option value=""></option>
											<?php
												for($i=0;$i<count($tier);$i++)
												{
											?>
                                            		<option value="<?=$tier[$i]['id']?>"><?=$tier[$i]['name']?></option>
                                            <?php		
												}
											?>
                                        </select>
                                ...
                                <div class="form-group">
                                    <label>Date</label>
                                    <input type="text"   class="form-control required" id="tanggal" name="tanggal" placeholder="Date " value="<?=isset($data['date'])?$data['date']:date('Y-m-d h:i:s')?>" />
                                </div>
...
$(document).ready(function(){
	$("#frm-object").validate({
		...
		submitHandler:function(){
			...
			var req = postFile('<?=site_url("plg/order/save")?>',data);
			req.done(function(out){
				if(!out.error)
				{
					smart_success_box(out.message,'#frm-object .card-body');
					document.location.href="<?=site_url('plg/order')?>";
				}
				else
				{
