                    ...
                    <div class="card">
                        ...
                        List 
                         ...
                         <span class="pull-right">
                               <a   href="<?=site_url('plg/order/add')?>" class="btn btn-sm btn-mini btn-xs pull-right" type="button" data-toggle="tooltip" title="" data-original-title="add"><i class="fa fa-plus"></i></a>
                         </span>  
						 */
						 ?>
                      </div>
                      <div class="card-body">
                      	  <div class="alert alert-warning">
                         	 <strong>Presale is now closed </strong>
                          </div>
...
<script>
var table_ajax;
var curls = "<?=site_url('plg/order/getlist')?>?baru=1";
...
$(document).ready(function(){
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('plg/order/delete')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
	...
	$("#frm-reset").validate({
		...
		submitHandler:function(){
			var req = post('<?=site_url('plg/order/reset-password')?>',$("#frm-reset").serialize());
			req.done(function(out){
				if(!out.error)
				{
	...
	$("#frm-status").validate({
		...
		submitHandler:function(){
			...
			curls = "<?=site_url('plg/order/getlist')?>?baru="+ id_search;
		...
		if(ids!="" && stores!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('plg/order/defaultuser')?>',{id:ids,"id_sites":stores,"default_store":checks,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
