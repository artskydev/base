    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="form-group">
          ...
          <div class="form-group hashed hide">
        	<label>HASH</label>
            <input type="text" class="form-control  "   name="txhash_wallet" id="txhash_wallet" placeholder="HASH Payment" />
        </div>
      </div>
...
<script>
var table_ajax;
var curls = "<?=site_url('order-presale/getlist')?>";
...
$(document).ready(function(){
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('order-presale/delete')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
	...
	$("#frm-status").validate({
		...
		submitHandler:function(){
			var req = post('<?=site_url('order-presale/status')?>',$("#frm-status").serialize());
			req.done(function(out){
				if(!out.error)
				{
					table_ajax.api().ajax.reload();
	...
	$(".status_pay").on("change",function()
	{
		...
		if(types==1)
		{
			//$(".hashed").removeClass("hide");	
		}else
		{
			 
			$(".hashed").addClass("hide");
			$("#txhash_wallet").val();	
		}
	});
});
