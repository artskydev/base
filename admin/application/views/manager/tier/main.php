                    ...
                    <div class="card">
                        ...
                        List Admin
                         <span class="pull-right">
                          	 ...
                          	 <a   href="<?=site_url('tier/add')?>" class="btn btn-sm btn-mini btn-xs pull-right" type="button" data-toggle="tooltip" title="" data-original-title="add"><i class="fa fa-plus"></i></a>
                         </span>  
                      </div>
                      <div class="card-body">
...
<script>
var table_ajax;
var curls = "<?=site_url('tier/getlist')?>?baru=1";
...
$(document).ready(function(){
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('tier/delete')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
	...
	$("#frm-reset").validate({
		...
		submitHandler:function(){
			var req = post('<?=site_url('tier/reset-password')?>',$("#frm-reset").serialize());
			req.done(function(out){
				if(!out.error)
				{
	...
	$("#frm-status").validate({
		...
		submitHandler:function(){
			...
			curls = "<?=site_url('tier/getlist')?>?baru="+ id_search;
		...
		if(ids!=""  )
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('tier/defaults')?>',{id:ids,'displays':displays ,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
	...
	$(".js-dataTable-full").on('click','.btn-closeds',function(){
		var ids = $(this).data('ref');
		var ends = $(this).data('ends'); 
		...
		if(ids!=""  )
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('tier/endsv')?>',{id:ids,'ends':ends ,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
