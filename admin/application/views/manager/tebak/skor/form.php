	...
	<div class="col-md-12">
   	   ...
   	   <div class="card">
            ...
            <div class="card-body">
                                     ...
                                     <div class="form-group">
                                        <label>MATCH SKOR FROM CUSTOMER</label>
                                        <p>
                                        	<i>
											<?=$match['team1']?> (<?=$data['skor_team1']?>) vs <?=$match['team2']?> (<?=$data['skor_team2']?>) 
                                            </i>
                                        </p>
                                    </div>
                                    <div class="form-group">
...
$(document).ready(function(){
	$("#frm-object").validate({
		...
		submitHandler:function(){
			...
			var req = postFile('<?=site_url("tebak/skor/save")?>',data);
			req.done(function(out){
				if(!out.error)
				{
					smart_success_box(out.message,'#frm-object .block-content');
					document.location.href="<?=site_url('tebak/skor')?>";
				}
				else
				{
