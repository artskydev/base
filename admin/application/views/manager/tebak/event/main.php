                    ...
                    <div class="card">
                        ...
                        List <?=$title?>
                         <span class="pull-right">
                          	 ...
                          	 <a   href="<?=site_url('tebak/event/add')?>" class="btn btn-sm btn-mini btn-xs pull-right" type="button" data-toggle="tooltip" title="" data-original-title="add"><i class="fa fa-plus"></i></a>
                         </span>  
                      </div>
                      <div class="card-body">
...
<script>
var table_ajax;
var curls = "<?=site_url('tebak/event/getlist')?>?baru=1";
...
$(document).ready(function(){
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('tebak/event/delete')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
	...
	$("#frm-reset").validate({
		...
		submitHandler:function(){
			var req = post('<?=site_url('tebak/event/reset-password')?>',$("#frm-reset").serialize());
			req.done(function(out){
				if(!out.error)
				{
	...
	$("#frm-status").validate({
		...
		submitHandler:function(){
			...
			curls = "<?=site_url('tebak/event/getlist')?>?baru="+ id_search;
		...
		if(ids!="" && stores!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('tebak/event/default')?>',{id:ids,"id_sites":stores,"default_store":checks,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
