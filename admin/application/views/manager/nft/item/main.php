                           ...
                           <table class="table table-bordered table-striped js-dataTable-full">
                            <thead>
                                <tr>
                                    ...
                                    <th>Price (<?=settings('token_name')?>)</th>
                                    <th>Type </th>
                                    <th>Image</th>
                                    <th>Set Home</th>
                                    <th>Token ID</th>
...
<script>
var table_ajax;
var curls = "<?=site_url('nft/nft-supply/getlist')?>?baru=1";
...
$(document).ready(function(){
	...
	$(".js-dataTable-full").on('click','.btn-delete-sites',function(){
		...
		if(ids!="")
		{
			bootbox.confirm({
				title: "Delete this ?",
				message: "Do you want to delete this <br/><small>(note: <strong>this delete only in apps not block explorer</strong>)</small>",
				buttons: {
					cancel: {
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('nft/nft-supply/delete')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
	...
	$(".js-dataTable-full").on('click','.btn-checked',function(){
		...
		{
			bootbox.confirm({
				title: "Set Banner Home ?",
				message: "Do you want to Set Banner Home  .",
				buttons: {
					cancel: {
						label: '<i class="fa fa-times"></i> Cancel'
					},
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('nft/nft-supply/defaults')?>',{id:ids,'homes':displays ,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
